    ...
    public function pay()
    {
        ...
        if (Session::has('payment_type')) {
            if (Session::get('payment_type') == 'cart_payment') {
                ...
                $merchantTransactionId = Session::get('payment_type') . '-' . $combined_order->id . '-' . auth()->id() . '-' . rand(0, 100000);
                $merchantUserId = auth()->id();
            } elseif (Session::get('payment_type') == 'wallet_payment') {
                $amount = Session::get('payment_data')['amount'];
                $merchantTransactionId = Session::get('payment_type') . '-' . auth()->id() . '-' . auth()->id() . '-' . rand(0, 100000);
                $merchantUserId = auth()->id();
            } elseif (Session::get('payment_type') == 'customer_package_payment') {
                $customer_package = CustomerPackage::findOrFail(Session::get('payment_data')['customer_package_id']);
                $amount = $customer_package->amount;
                $merchantTransactionId = Session::get('payment_type') . '-' . $customer_package->id . '-' . auth()->id() . '-' . rand(0, 100000);
                $merchantUserId = auth()->id();
            } elseif (Session::get('payment_type') == 'seller_package_payment') {
                $seller_package = SellerPackage::findOrFail(Session::get('payment_data')['seller_package_id']);
                $amount = $seller_package->amount;
                $merchantTransactionId = Session::get('payment_type') . '-' . $seller_package->id . '-' . auth()->id() . '-' . rand(0, 100000);
                $merchantUserId = auth()->id();
            }
        }
